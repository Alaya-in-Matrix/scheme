...
(define (install-polynomial-package)
  ...
  (begin
	"debug"
	(put 'mul-term-by-all-terms '(term term-list)
				  ...
				  (let ((o (order t))
					...
					(let ((t1 (make-term 0
										 (make-polynomial now-var (adjoin-term (make-term o 1)
																			   (the-empty-termlist)))))
						  (L1 (if (equal? (type-tag c) 'polynomial)		;;not satisfying style
  ...
  (put 'coerce-poly '(polynomial var) coerce-poly)
  (define (tag p)
...
;;							 
(define (coerce-poly p var)
  ((get 'coerce-poly '(polynomial var)) (contents p) var))
(define (mul-term-by-all-terms t L)
  ((get 'mul-term-by-all-terms '(term term-list)) t L))
